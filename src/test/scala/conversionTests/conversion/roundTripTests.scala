...
//  }
//
//  def readAndWriteTriples(inputFile:File, tempDir:File): File = {
//    val format = FileUtil.getFormatType(inputFile,"")
//    val triples = new TripleHandler().read(inputFile.pathAsString, format)
//
//    if(format=="rdf") new TripleHandler().write(triples, "rdfxml")
//    else new TripleHandler().write(triples, format)
//
...
//      else '\t'
//    }
//    val dataFrame = new TSDHandler().read(inputFile.pathAsString, format, delimiter)
//
//    new TSDHandler().write(dataFrame, format, delimiter)
//
//    val targetFile = tempDir.parent / inputFile.name
//
//    try {
